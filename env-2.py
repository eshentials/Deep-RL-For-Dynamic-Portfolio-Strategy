...
BULL_EXPOSURE_FLOOR: float  = 0.99
...
REGIME_REWARD_PARAMS: dict[str, dict] = {
    "bull": dict(
        alpha_weight    = 7.5,   # [BF-10]; effective growth scale = 7.5 * R_LR_MULTIPLIER = 15.0
        drawdown_weight = 0.05,  # effective MDD scale = 0.05 * MDD_PENALTY_SCALE = 0.025
        exposure_weight = 0.15,  # [BF-4]; soft one-sided; hard floor via BULL_FLOOR_PENALTY_SCALE
        exposure_target = 1.00,
        turnover_weight = 0.01,  # effective = 0.01 * GLOBAL_TURNOVER_RELIEF * (1 - [LA-4])
    ),
    "bear": dict(
        alpha_weight    = 4.0,
...
class PortfolioEnv(gym.Env):
    ...
    def _get_regime(self, obs: np.ndarray) -> str:
        ...
        bull_score = (
            2.0 * max(momentum, 0.0)
            + 0.5 * max(macro_regime, 0.0)
            - 0.5 * max(vol - 0.020, 0.0)
        )
        if bull_score >= 1.0:
            return "bull"
    ...
    def step(
        ...
        if regime == "bull":
            ...
            momentum_bonus = MOMENTUM_ALIGNMENT_SCALE * max(momentum_alignment, 0.0)
        else:
            momentum_bonus = 0.0
        ...
        if regime == "bull":
            ...
            effective_turnover_weight = base_turnover_weight * (
                1.0 - BULL_LOW_LAM_TURNOVER_RELIEF * aggression_frac          # [LA-4]
            )
        else:
            effective_turnover_weight = base_turnover_weight
        ...
        reward = (
            ...
            + participation_bonus                            # [BF-6][RT-4] zero non-bull
            + momentum_bonus                                 # [BF-7] zero in non-bull
